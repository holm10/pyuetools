...
def it(Y,s=None,ax=False,line='-',marker='o',color='k',linewidth=2,markersize=8,xlabel=None,ylabel=None,title="IT profile",xlim=False,ylim=False,yaxis="lin",legend=False,xaxis='it',marksep=True):
    ...
    ret=radialplotter(x,Y,ax,line,marker,color,linewidth,markersize,xlabel,ylabel,title,xlim,ylim,yaxis,legend,marksep)
...
def row(Y,row,s=None,ax=False,line='-',marker='o',color='k',linewidth=2,markersize=8,xlabel=None,ylabel=None,title=None,xlim=False,ylim=False,yaxis="lin",xaxis="mp",legend=False,marksep=True):
    ...
    ret=radialplotter(x,Y[row,:],ax,line,marker,color,linewidth,markersize,xlabel,ylabel,title,xlim,ylim,yaxis,legend,marksep)
...
def plot(X,Y,ax=False,linewidth=2,markersize=8,xlabel=False,ylabel=False,title=False,xlim=False,ylim=False,yaxis="lin",marker='',line='-',color='k',legend=False,xaxis='lin',gc=False):
    ...
    ax[=False]:     Axis where to plot. If False, new figure is created. If ax is figure object, plots on first axes object. 
                    If ax is axes object, plots on the requested axes.
    gc[=False]      Switch whether to omit guard cells (first and last values)
    line[='-']:     Line style
    marker[='']     Marker style
    color[='k']     Marker and line color
    linewidth[=2]   Plotted line width
    ...
    if yaxis=="lin":
        if xaxis=="lin":
            ...
            ax.semilogx(X,Y,color=color,marker=marker,linestyle=line,linewidth=linewidth,markersize=markersize)
        else:
        ...
        if xaxis=="lin":
            ...
            ax.loglog(X,Y,color=color,marker=marker,linestyle=line,linewidth=linewidth,markersize=markersize)
        else:
            print("Specify xaxis to be either 'lin' or 'log'!")
            return
    else:
